...
// Second variation with spaces might exist
const docEndStrAlt = `                             </div>
                           )}
                         </div>
                       </div>
...
const replaceDocEndStrAlt = `                             </div>
                           )}
                       </div>

                       {/* Property Footer`;

if (code.indexOf(docEndStrAlt) > -1) {
  code = code.replace(docEndStrAlt, replaceDocEndStrAlt);
  console.log("Chunk 2 Alt success");
}
